...
{   
    ...
    fwrite(&aluno, sizeof(disciplina), 1, file);
...
{   
    ...
    for (i; i < 40; i++){
        if(aluno->turma[i].discente.matricula == 0){
            ...
            printf("\n\n");
            fwrite(&aluno->turma[i].discente, sizeof(disciplina), 1, file);
            break;
        }
        else{
            c ++;
...
{	
    ...
    for (i = 0; i < 40; i++){
        if (aluno->turma[i].discente.matricula != 0){
            ...
            fwrite(&aluno->turma[i].discente, sizeof(disciplina), 1, file2);
        }    
    }
    printf("\n\n");
}
...
int main(void)
{
    ...
    disciplina aluno;
    remove("alunos.dat"); remove("boletins.dat");
    do
    {   
        if (inicializar == 1)
        {
